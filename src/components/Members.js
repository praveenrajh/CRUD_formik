...
function Members(props) {
  ...
  if(props.members.length !=0){
   const setData=props.members.map((person)=>{
        return createData(person.name, person.image,person.age,person.phone,person.address,person.joindate,person.books,person.id)
       })
       rows = setData;
  }

  const handleDelete=(id)=>{
        axios.delete(`https://636c8f127f47ef51e14ba6ab.mockapi.io/members/${id}`)
        .then((res)=>{
          fetch()
          ...
          {rows.map((row) => (
            <StyledTableRow key={row.id}>
              ...
              <StyledTableCell align="right">
                <Button color='error' onClick={()=>handleDelete(row.id)}>del</Button>
